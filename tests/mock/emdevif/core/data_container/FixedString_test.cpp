...
TEST(FixedStringTest, ElementAccess) {
    ...
    EXPECT_EQ(s[0], 'x'); EXPECT_EQ(s[1], 'y'); EXPECT_EQ(s[2], 'z');
    EXPECT_EQ(s.front(), 'x'); EXPECT_EQ(s.back(), 'z');
}
...
TEST(FixedStringTest, Comparison) {
    ...
    EXPECT_TRUE(a == b); EXPECT_TRUE(a < c);
}
...
TEST(FixedStringTest, EmptyString) {
    ...
    static_assert(s.empty()); static_assert(s.size() == 0);
}
