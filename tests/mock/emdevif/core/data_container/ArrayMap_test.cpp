...
TEST(ArrayMapTest, ConstructTest) {
    ...
    for (int cnt = 1; auto& e : map1) { auto [k, v] = e; EXPECT_EQ(cnt, v); ++cnt; }
    decltype(map1) map2 = map1;
    for (int cnt = 1; auto& e : map2) { auto [k, v] = e; EXPECT_EQ(cnt, v); ++cnt; }
    map1["three"] = 333; map1["four"] = 444;
...
TEST(ArrayMapTest, EraseIfTest) {
    ...
    auto count = erase_if(map, [](const auto& item) { auto const& [key, value] = item; return (key % 2) != 0; });
    EXPECT_EQ(count, 3u);
    EXPECT_EQ(map.size(), 2u);
    constexpr auto expect_list = std::to_array<std::pair<int, char>>({{2, 'b'}, {4, 'd'}});
    auto idx = 0_zu;
    for (auto& e : map) { auto [k, v] = e; EXPECT_EQ(expect_list[idx].first, k); ++idx; }
}

TEST(ArrayMapTest, KeyEqualTParamTest) {
    struct constCharPtrEq { bool operator()(char const* l, char const* r) const { return std::strcmp(l, r) == 0; } };
