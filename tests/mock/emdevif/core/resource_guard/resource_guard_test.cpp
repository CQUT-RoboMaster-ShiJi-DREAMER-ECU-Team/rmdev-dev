...
static int g_cnt = 0;
void inc() noexcept { ++g_cnt; }
void reset() noexcept { g_cnt = 0; }

TEST(DeferTest, BasicExecution) {
    reset(); { Defer d(&inc); EXPECT_EQ(g_cnt, 0); } EXPECT_EQ(g_cnt, 1);
}
TEST(DeferTest, LambdaCapture) {
    int v = 0; { Defer d([&v]() noexcept { v = 42; }); } EXPECT_EQ(v, 42);
}
TEST(DeferTest, ReverseOrder) {
    ...
    EXPECT_EQ(ord[0],3); EXPECT_EQ(ord[1],2); EXPECT_EQ(ord[2],1);
}
TEST(DeferTest, NestedDefer) {
    int v = 0;
    { Defer d1([&v]() noexcept { v += 10; });
      { Defer d2([&v]() noexcept { v += 1; }); EXPECT_EQ(v, 1); } }
    EXPECT_EQ(v, 11);
}
...
public:
    ...
    ErrorCode lock() noexcept { ++lk; return lret; }
    ErrorCode lock(int) noexcept { ++lk; return lret; }
    ErrorCode try_lock() noexcept { ++tlk; return tlret; }
    void unlock() noexcept { ++ulk; }
};

#ifdef __cpp_exceptions
TEST(LockGuardTest, AutoLock) {
    MockLock lock;
    { LockGuard guard(lock); EXPECT_EQ(lock.lk,1); } EXPECT_EQ(lock.ulk,1);
}
#endif
TEST(LockGuardTest, DoNotLock) {
    MockLock lock;
    { LockGuard guard(lock_guard_do_not_lock_when_init, lock); EXPECT_EQ(lock.lk,0); } EXPECT_EQ(lock.ulk,1);
}
TEST(LockGuardTest, ManualLock) {
    ...
    { LockGuard guard(lock_guard_do_not_lock_when_init, lock);
      ...
      EXPECT_EQ(lock.lk,1); } EXPECT_EQ(lock.ulk,1);
}
...
public:
    ...
    int ia=0;
    ErrorCode init() noexcept { initd=true; return ire; }
    ErrorCode init(int a) noexcept { initd=true; ia=a; return ire; }
    void deInit() noexcept { deinitd=true; }
};

#ifdef __cpp_exceptions
TEST(InitGuardTest, AutoInit) {
    MockDevice dev;
    { InitGuard guard(dev); EXPECT_TRUE(dev.initd); } EXPECT_TRUE(dev.deinitd);
}
#endif
TEST(InitGuardTest, DoNotInit) {
    MockDevice dev;
    { InitGuard guard(init_guard_do_not_init_object_tag, dev); EXPECT_FALSE(dev.initd); } EXPECT_TRUE(dev.deinitd);
}
