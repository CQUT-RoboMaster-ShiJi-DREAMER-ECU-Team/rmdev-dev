...
> 这个仓库不是产品工程模板，而是把各个独立的库子模块聚拢到一起，做集成构建、宿主单元测试与文档聚合。库本身可以脱离本仓库独立使用。

## 这个仓库是什么

`emdevif`（嵌入式开发接口抽象）和 `rmdev`（RoboMaster 电控开发库）都是**独立维护**的库，各有自己的 Git 仓库、版本历史与 CI。它们各自都能被单独克隆、集成到任何工程中。

`rmdev-dev` 把它们以 git submodule 的形式聚拢在一起，提供：

- 统一的宿主（Windows / Linux / macOS）单元测试环境
- 跨子模块的集成构建验证（含 C++20 模块 ON/OFF 双路径）
- 统一的编码规范与开发流程文档
- 代码格式化、测试运行等辅助脚本
...
```
rmdev-dev/
├── rmdev/                        # 电控算法/驱动聚合库（独立仓库，子模块）
├── emdevif_collection/
│   ├── emdevif/                  # 嵌入式通用接口抽象（独立仓库，子模块）
│   └── emdevif_stm32_peripheral/ # emdevif 的 STM32 外设扩展（独立仓库，子模块）
├── tests/                        # 宿主单元测试（GoogleTest）
├── docs/
...
每个子模块目录下都有自己的 `README.md`（面向人类的使用指南）和 `AGENTS.md`（面向协作智能体的约束说明）。

## 快速开始：运行宿主单元测试

本仓库目前实现的验证路径是宿主单元测试（`TEST_PLATFORM=mock`），不依赖任何板卡或工具链。
...
# 构建并测试（以 HostTestModulesExceptions 为例，支持 Debug / Release）
...
> 提示：子模块文档里出现的「编码规范」GitHub 外链，指向的就是本仓库的 `docs/coding-style.md`。既然你已经在 `rmdev-dev` 里，直接看本地这份即可。

## 克隆与初始化

```bash
git clone --recurse-submodules https://github.com/CQUT-RoboMaster-ShiJi-DREAMER-ECU-Team/rmdev-dev.git
```
