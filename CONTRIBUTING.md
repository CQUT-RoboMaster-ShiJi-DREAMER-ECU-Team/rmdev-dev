...
`rmdev-dev` 是 `emdevif` 与 `rmdev` 的开发与测试集成环境。本文档面向所有贡献者（人类与协作智能体），说明如何参与本仓库以及上下游子模块的协作。

## 仓库定位

`rmdev-dev` **不是一个产品工程模板**，而是将独立维护的库子模块聚拢到一起，提供集成构建、模拟单元测试与文档聚合的环境。
...
3. 确保改动通过模拟单元测试（见 [README.md](README.md) 快速开始章节）。
